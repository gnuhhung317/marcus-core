...
/**
 * Spring Data repository for append-only raw event log.
 * Ensures idempotency and enables efficient replay queries.
 */
public interface SpringDataRawEventRepository extends JpaRepository<RawEventEntity, String> {
    ...
    /**
     * Find all unprocessed raw events for a bot, ordered by sequence.
     * Used by the projection worker to catch up.
     */
    List<RawEventEntity> findByBotIdAndProcessedFalseOrderBySequenceNoAsc(String botId);

    /**
     * Find raw events by botId and sequence range (inclusive).
     * Used for replay-request handling.
     */
    @Query(
    ...
    /**
     * Find raw events by botId and time range.
     * Used for replay-request with timestamp-based filtering.
     */
    @Query(
        "SELECT r FROM RawEventEntity r " +
    ...
    /**
     * Get the maximum sequence number for a botId.
     * Used to assign the next sequence number on ingest.
     */
